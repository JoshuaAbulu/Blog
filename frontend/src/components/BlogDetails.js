...
const BlogDetails = ({blog}) => {
    ...
    return ( 
        <div className="flex flex-col border-b-yellow-400 border-b-4 rounded-2xl w-4/5 items-center bg-zinc-900">
                {blog.image && <img src={blog.image} alt="blogimg" className=" w-full rounded-t-2xl" />}
                <div className="w-4/5 flex flex-col pt-8 gap-4">
                    <h2 className="text-2xl font-black text-justify">{blog.title}</h2>
                    <h3 className=" text-yellow-400 text-xl">Posted {formatDistanceToNow(new Date(blog.createdAt), {addSuffix: true})}</h3>
